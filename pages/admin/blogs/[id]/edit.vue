<template>
  <div class="max-w-4xl mx-auto space-y-6">
    ...
    <div v-else-if="error" class="text-center py-12">
      ...
      <NuxtLink to="/admin/blogs" class="utopia-button-primary">
        Back to Articles
      </NuxtLink>
    </div>
...
// Meta and layout
useSeoMeta({
  title: 'Edit Article - Admin - Utopia Minds',
...
const loadBlog = async () => {
  ...
  try {
    const result = await blogStore.fetchBlogById(blogId)
...
const submitForm = async () => {
  ...
  try {
    ...
    const result = await blogStore.updateBlog(blogId, blogData)
...
const autoSave = () => {
  ...
  autoSaveTimeout = setTimeout(async () => {
    if (form.value.status === 'draft' && !saving.value) {
